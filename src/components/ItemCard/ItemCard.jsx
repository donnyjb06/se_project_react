...
const ItemCard = ({ item }) => {
	const { handleToggleLike: toggleLike, handleCardClick } = useClothingData();
	...
	const handleToggleLike = async () => {
		try {
			await toggleLike(item._id, isLiked);
		} catch (error) {
	...
	const openModal = (e) => {
		if (e.target.closest(".item-card__like-toggle")) {
			return;
		}
	...
	return (
		<li className="item-card" onClick={openModal}>
			<div className="item-card__header">
				<p className="item-card__name">{item.name}</p>
				{isLoggedIn && (
					<button className="item-card__like-toggle" onClick={handleToggleLike}>
						<svg
							width="20"
							height="18"
							viewBox="0 0 20 18"
